...
Deno.serve(async (req) => {
  ...
  try {
    const AI_API_TOKEN = Deno.env.get("AI_API_TOKEN_e0bb6c9e1e23");
    if (!AI_API_TOKEN) {
    ...
    if (!body.model) {
      return jsonResponse(400, { success: false, message: "model is required", code: "invalid_request_error" });
    }
    if (!body.prompt?.trim()) {
    ...
    if (!response.ok) {
      ...
      return jsonResponse(response.status, { success: false, message, code: rawCode });
    }
